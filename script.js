...
let allPlants = []; // This will now be populated directly
let filteredPlants = [];
let currentPage = 1;
const itemsPerPage = 8; // Display 8 plants per page
...
// Get DOM elements for plants.html (if present)
...
function updateCartCount() {
    ...
    if (cartCountElement) {
        ...
        cartCountElement.style.display = totalItems > 0 ? 'block' : 'none'; // Show/hide
    }
...
function addToCart(plantId) {
    ...
    if (plant) {
        ...
        updateCartCount();
        renderCartItems(); // Update modal immediately
...
function renderCartItems() {
    ...
    if (cart.length === 0) {
        ...
        // Add event listeners for quantity and remove buttons in the modal
...
function populateFilterOptions() {
    ...
    const categories = [...new Set(allPlants.map(plant => plant.category))].sort(); // Sort categories alphabetically
    ...
    // Populate Price Filter (you can customize these ranges)
...
function displayPlants(plantsToDisplay) {
    ...
    document.querySelectorAll('.add-to-cart-btn').forEach(button => {
        button.addEventListener('click', (event) => {
            ...
            if (!isNaN(plantId)) { // Ensure plantId is a valid number
                 addToCart(plantId);
            } else {
...
function updatePlantDisplay() {
    ...
    displayPlants(plantsForCurrentPage);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (checkoutBtn) {
        checkoutBtn.addEventListener('click', () => {
            if (cart.length > 0) {
                ...
                // In a real app, navigate to checkout page or process order
    ...
    loadPlantsDirectly(); // Load plants data directly instead of fetching

    // Add event listeners for filters and search ONLY if on plants.html
    if (plantListing && categoryFilter) { // Check if these elements exist on the page
    ...
    // --- Hero Slider functionality (for index.html, if present) ---
    const sliderContainer = document.querySelector('.slider-container');
    if (sliderContainer) { // Check if slider exists on the page
